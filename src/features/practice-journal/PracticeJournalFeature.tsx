...
import type { PracticeLog } from '../../shared/context/StatsContext';
...
export default function PracticeJournalFeature() {
  ...
  const exerciseRows = ALL_EXERCISE_IDS.map(id => {
    const bestStreak = exerciseStats[id]?.bestStreak ?? 0;
    ...
    const daysPracticed = Object.keys(log).filter(date => {
      const day = log[date] as DayRecord | undefined;
      return ((day?.[id] as ScoreRecord | undefined)?.total ?? 0) > 0;
    }).length;
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <style>{`
        .pj-card { background:#161b22; border:1px solid #21262d; border-radius:12px; padding:24px; margin-bottom:20px; }
        .pj-section-title { font-size:11px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:#4b5563; margin-bottom:16px; }
        .pj-stat-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(140px,1fr)); gap:16px; }
        .pj-stat-box { background:#1c2128; border:1px solid #30363d; border-radius:10px; padding:16px; text-align:center; }
        .pj-stat-value { font-size:28px; font-weight:800; color:#e6edf3; line-height:1; margin-bottom:4px; }
        .pj-stat-label { font-size:11px; color:#6b7280; font-weight:500; }
        .pj-calendar { display:flex; gap:4px; flex-wrap:wrap; }
        .pj-day-dot { width:20px; height:20px; border-radius:4px; flex-shrink:0; }
        .pj-exercise-row { display:flex; align-items:center; gap:12px; padding:12px 0; border-bottom:1px solid #21262d; }
        .pj-exercise-row:last-child { border-bottom:none; }
        .pj-exercise-icon { width:36px; height:36px; border-radius:8px; background:#1c2128; border:1px solid #30363d; display:flex; align-items:center; justify-content:center; color:#7c3aed; flex-shrink:0; }
        .pj-exercise-name { font-size:14px; font-weight:600; color:#e6edf3; }
        .pj-exercise-meta { font-size:12px; color:#6b7280; margin-top:2px; }
        .pj-exercise-stats { margin-left:auto; text-align:right; flex-shrink:0; }
        .pj-badge { display:inline-flex; align-items:center; gap:4px; padding:3px 8px; border-radius:20px; font-size:12px; font-weight:600; }
        .pj-empty { text-align:center; padding:40px 20px; color:#4b5563; }
      `}</style>
      ...
      {statsLoading ? (
        <div style={{ textAlign: 'center', padding: 60, color: '#4b5563' }}>Loading stats…</div>
      ) : (
        <>
          {/* Streak + Summary */}
          <div className="pj-card">
            ...
            <div className="pj-stat-grid">
              <div className="pj-stat-box">
                <div className="pj-stat-value" style={{ color: todayTotal > 0 ? '#7c3aed' : '#4b5563' }}>{todayTotal}</div>
          ...
          <div className="pj-card">
            ...
            {exerciseRows.length === 0 ? (
              <div className="pj-empty">
                ...
                <div style={{ fontSize: 14, color: '#6b7280' }}>Complete some ear training exercises and your stats will appear here</div>
              </div>
            ) : (
              exerciseRows.map(s => (
