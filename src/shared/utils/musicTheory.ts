...
export function normalizeNoteName(note: string): NoteName | null {
  ...
  note = note.replace(/♯/g, '#').replace(/♭/g, 'b').replace(/♮/g, '');
...
/**
 * Traspone una nota di N semitoni - FIXED
 */
export function transposeNote(noteName: NoteName, semitones: number): NoteName {
  ...
  // FIX: Gestisci correttamente anche semitoni negativi o > 12
  // Usa ((x % n) + n) % n per modulo sempre positivo
...
/**
 * Costruisce un accordo dato root e intervalli - FIXED
 */
export function buildChordFromIntervals(root: NoteName, intervals: number[]): NoteName[] {
  return intervals.map((semitones) => {
    // Riduci l'intervallo all'interno di un'ottava (0-11)
    const semitonesInOctave = semitones % 12;
    return transposeNote(root, semitonesInOctave);
  });
}
